...
cum_lags <- read_csv(paste0(output.folder, "cumm_lags_OR.csv")) %>%
  ...
  dplyr::select(-lag) %>%
  mutate(model = "cumm_exp",
         parish.SES = if_else(lag_constr == "3-df" | lag_constr == "4-df","Parish SES nonadjusted", "Parish SES adjusted")) %>%
...
#dplyr::filter(model %in% c("3-df with/parish ses", "Parish SES adjusted")) %>%
 ...
 # scale_y_continuous(limits = c(0.95, 1.10), breaks = seq(.95, 1.10, by = 0.05)) +
  ...
  scale_linetype_manual(labels = c("Parish-SES adjusted", "Parish-SES non-adjusted"), values = c("solid", "dashed"),
...
### 2b. NOT adjusted for parish-level SES
...
dplyr::filter(model %in% c("3-df", "Parish SES nonadjusted")) %>%
 ...
 # scale_y_continuous(limits = c(0.95, 1.10), breaks = seq(.95, 1.10, by = 0.05)) +
  ...
  scale_color_manual(labels = c("Cum. Exposure","Averg. Exposure"), values = c("#1b9e77", "orange3"))
...
This figure is Figure 2 in the manuscript. It shows the lag effect estimates from the distributed lag model adjusted for parish SES with 3-df in the lag constraint

```{r}


distr_lag <- distrib_lags %>%
  dplyr::filter(lag_constr %in% c("3-df w/parish ses", "3-df")) %>%
...
### 3c. 3df in lag constrain models NOT adjusted for parish-level SES
